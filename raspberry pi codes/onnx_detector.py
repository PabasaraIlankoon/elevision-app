"""
ONNX Elephant Detector for Raspberry Pi RW-001
Uses 320x320 input size for faster inference on Pi CPU.
No PyTorch or ultralytics needed.
"""
...
class ONNXElephantDetector:
    ...
    def _load_model(self):
        ...
        log.info(f'Using input size: {self.input_width}x{self.input_height} for fast Pi inference')
    ...
    def warmup(self):
        log.info('Running model warmup (first inference is slow)...')
        dummy = np.zeros((480, 640, 3), dtype=np.uint8)
        start = __import__('time').time()
        self.detect(dummy)
        elapsed = __import__('time').time() - start
        log.info(f'Warmup complete — inference time: {elapsed:.1f}s per frame')
        log.info(f'Expected LED response time: {elapsed:.1f}s after elephant appears')